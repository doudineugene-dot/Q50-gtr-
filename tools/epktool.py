#!/usr/bin/env python3
"""
Разбор, сборка и проверка контейнеров .epk головного устройства Infiniti Q50
(InTouch / DCU Gen1). Перенесено из doudineugene-dot/q50 без изменений в
криптографии: формат и алгоритм здесь те же, добавлены только явное имя файла
внутри контейнера (--name) и строгая команда verify для CI.
...
только чтобы распаковать чужой .epk.
    ...
    epktool.py build   app.apk -o app.epk --cert c.pem --name q50gtr.apk
    epktool.py build   a.apk b.apk -o bundle.epk --cert keys/obu_cert.pem
    epktool.py verify  app.epk --expect-name q50gtr.apk        # для CI
...
def cmd_build(args):
    ...
    with open(args.output, 'wb') as f:
        ...
        for i, p in enumerate(args.inputs):
            # Имя внутри контейнера: по умолчанию basename, но ГУ ждёт короткий
            # ASCII, поэтому его можно задать явно через --name.
            if args.name:
                if len(args.inputs) != 1:
                    sys.exit('--name можно задать только для одного входного файла')
                text = args.name
            else:
            ...
            try:
                ...
                sys.exit('имя внутри контейнера должно быть ASCII: %r' % text)
...
def cmd_verify(args):
    """Проверить контейнер против ожидаемой раскладки. Ненулевой код — провал.

    Нужна в CI: собранный .epk нельзя распаковать без закрытого ключа OBU,
    поэтому единственное, что здесь можно доказать, — что контейнер сложен
    ровно так, как ждёт загрузчик ГУ.
    """
    data = open(args.file, 'rb').read()
    checks = []
    ...
    check('magic == ".epk"', data[:4] == MAGIC, repr(data[:4]))

    if data[:4] != MAGIC:
        _report(args.file, data, checks)
        sys.exit(1)

    h = parse_envelope(data)
    check('version == %d' % VERSION, h['version'] == VERSION, str(h['version']))
    check('payloadType == %d' % args.expect_type,
          h['payload_type'] == args.expect_type, str(h['payload_type']))
    check('blockCount == %d' % args.expect_blocks,
          h['block_count'] == args.expect_blocks, str(h['block_count']))
    check('keySize == %d' % args.expect_key_size,
    ...
    for i, (name, blen, ct, _off) in enumerate(blocks):
        if args.expect_name is not None:
    ...
    computed = ENVELOPE + sum(NAME_FIELD + 4 + b for _, b, _, _ in blocks)
    check('размер контейнера сходится', computed == len(data),
          'посчитано %d, файл %d' % (computed, len(data)))
...
def _report(path, data, checks):
    print('проверка: %s (%d байт)' % (path, len(data)))
    print()
    ...
    if bad:
        print('ПРОВАЛ: не прошло проверок: %d из %d' % (bad, len(checks)))
    else:
        print('все проверки пройдены: %d' % len(checks))
...
def main():
    ...
    s.add_argument('--name', help='имя файла внутри контейнера (по умолчанию basename)')
    s.set_defaults(func=cmd_build)

    s = sub.add_parser('verify', help='строгая проверка .epk (для CI)')
    s.add_argument('file')
    s.add_argument('--expect-name', help='ожидаемое имя файла внутри контейнера')
    s.add_argument('--expect-type', type=int, default=2)
    s.add_argument('--expect-blocks', type=int, default=1)
    s.add_argument('--expect-key-size', type=int, default=128)
