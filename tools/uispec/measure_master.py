...
def spec(m, base):
    ...
    for lbl, col in [("Фон экрана", "#020408"), ("Фон навигации", "#000005"),
        ...
        w("| %s | `%s` |" % (lbl, col))
    w("")
    w("## 10. Как эталон переносится на реальный экран")
    w("")
    w("Эталон нарисован в пропорции 548x480 (1.14:1), экран головного")
    w("устройства шире: 800x480 по замерам проекта q50, то есть 1.67:1.")
    w("Первая реализация клала эталон растром целиком и вписывала его в окно —")
    w("на машине это дало чёрные поля по 131 px с каждой стороны и мыло от")
    w("двойного масштабирования (растр -> Canvas -> экран).")
    w("")
    w("Поэтому полноэкранный растр убран. Всё рисуется Canvas в конечном")
    w("размере окна:")
    w("")
    w("* вертикаль и все размеры — единый масштаб `s = высота окна / 480`;")
    w("* горизонталь — раздаётся по фактической ширине окна;")
    w("* радиусы приборов масштабируются тем же `s`, поэтому круги остаются")
    w("  кругами при любой ширине;")
    w("* карточки, статус-бар и навигация тянутся на всю ширину.")
    w("")
    w("Единственный оставшийся растр — вид Q50 сзади на вкладке ШАССИ")
    w("(`res/drawable-nodpi/q50_rear.png`, 198x138 из MASTER_CHASSIS). Его")
    w("нельзя нарисовать вектором: на эталоне это фотография. Увеличивается он")
    w("не более чем примерно в полтора раза, с билинейной фильтрацией, и края")
    w("сведены в прозрачность, чтобы не был виден прямоугольник кропа.")
    w("")
    w("## 11. Про попиксельное сравнение")
    w("")
    w("`tools/uispec/compare.py` сравнивает рендер с эталоном. Пока интерфейс")
    w("занимал ровно рамку эталона, это была осмысленная метрика: MAE доходил")
    w("до 2.83 при SSIM 0.93.")
    w("")
    w("После перехода на полную ширину геометрия отличается намеренно, и")
    w("попиксельное сравнение измеряло бы уже не качество реализации, а саму")
    w("разницу раскладок. Соответствие эталону теперь проверяется поэлементно:")
    w("те же приборы, те же шкалы и диапазоны, те же карточки и подписи, те же")
    w("цвета, те же три вкладки — но на всю ширину экрана.")
    w("")
    return "\n".join(L) + "\n"
